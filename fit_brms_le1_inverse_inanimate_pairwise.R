...
trial_bins_path <- ifelse(
  length(args) >= 1,
  args[[1]],
  "output/koryak_number_animacy_sentence_graphs/speech_planning_trial_bins.csv"
)
output_dir <- ifelse(length(args) >= 2, args[[2]], "output/koryak_brms_le1_inverse_inanimate_pairwise")
target_window_key <- ifelse(length(args) >= 3, args[[3]], Sys.getenv("KORYAK_BRMS_WINDOW_KEY", unset = "linguistic_encoding_1"))
window_label <- ifelse(length(args) >= 4, args[[4]], Sys.getenv("KORYAK_BRMS_WINDOW_LABEL", unset = "le1"))
...
contrast_id <- "inverse_2_2_inanimate_vs_inverse_2_1_inanimate"
condition_a <- "inverse_2_agents_2_patients_inanimate_patient"
condition_b <- "inverse_2_agents_1_patient_inanimate_patient"
label_a <- "inverse 2 agents, 2 patients, inanimate patient"
label_b <- "inverse 2 agents, 1 patient, inanimate patient"
...
write_csv(
  ...
  file.path(output_dir, paste0("brms_", window_label, "_inverse_inanimate_pairwise_counts.csv"))
)
...
write_csv(summaries, file.path(output_dir, paste0("brms_", window_label, "_inverse_inanimate_pairwise_fixed_effects.csv")))
write_csv(diagnostics, file.path(output_dir, paste0("brms_", window_label, "_inverse_inanimate_pairwise_diagnostics.csv")))
write_csv(all_diagnostics, file.path(output_dir, paste0("brms_", window_label, "_inverse_inanimate_pairwise_all_parameter_diagnostics.csv")))
write_csv(sampler_diag, file.path(output_dir, paste0("brms_", window_label, "_inverse_inanimate_pairwise_sampler_diagnostics.csv")))
...
cat("Koryak LE1 Bayesian pairwise model: inverse inanimate patients\n")
cat("================================================================\n\n")
...
cat("Counts:\n")
print(read_csv(file.path(output_dir, paste0("brms_", window_label, "_inverse_inanimate_pairwise_counts.csv")), show_col_types = FALSE))
cat("\nFixed effects:\n")
print(summaries)
...
message("Wrote brms ", window_label, " inverse inanimate pairwise outputs to: ", output_dir)
