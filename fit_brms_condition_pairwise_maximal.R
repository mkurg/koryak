...
if (length(args) < 9) {
  stop(
    "Usage: Rscript fit_brms_condition_pairwise_maximal.R ",
...
model_df <- raw |>
  ...
  mutate(
    ...
    # Item slopes for condition need the lexical/stimulus item, not the condition-specific image file.
    item = factor(sub("-[0-9]+-[0-9]+-[lr]\\.png$", "", image)),
...
output_prefix <- paste0("brms_", window_label, "_", contrast_id, "_maximal")
...
write_csv(
  tibble(
    ...
    item_definition = "lexical stimulus extracted from image filename"
  ),
...
write_csv(model_df, file.path(output_dir, paste0(window_label, "_", contrast_id, "_maximal_model_data.csv")))

agent_formula <- bf(
  log.agent | weights(1 / wts.agent) ~ poly(time, degree = 3) * contrast_code +
    (1 + poly(time, degree = 3) * contrast_code | participant) +
    (1 + poly(time, degree = 3) | participant_condition) +
    (1 + poly(time, degree = 3) * contrast_code | item)
)
...
message("Fitting ", contrast_id, " maximal agent looks")
fit_agent <- brm(
...
message("Fitting ", contrast_id, " maximal patient looks")
fit_patient <- brm(
...
cat("Koryak Bayesian maximal pairwise gaze model\n")
cat("===========================================\n\n")
cat("Window:", window_label, "(", target_window_key, ")\n")
cat("Comparison:", label_a, "vs", label_b, "\n")
cat("Coding: +1 =", label_a, "; -1 =", label_b, "\n")
cat("Item definition: lexical stimulus extracted from image filename.\n")
cat("Random effects: correlated intercepts/slopes for cubic time, contrast, and time-by-contrast by participant and item; correlated intercept/time slopes by participant-condition.\n\n")
cat("Counts:\n")
print(counts)
cat("\nFixed effects:\n")
print(summaries)
...
message("Wrote maximal brms ", window_label, " ", contrast_id, " outputs to: ", output_dir)
